...
<head>
    ...
    @php
        // En papel el dinero lleva separador de miles: seis dígitos seguidos no
        // se leen de un vistazo, y esta hoja se revisa de un vistazo.
        $d = fn (int $centavos) => number_format($centavos / 100, 2, '.', ',');
        // Un cero no lleva signo: "−0.00" se lee como un descuido.
        $resta = fn (int $centavos) => ($centavos > 0 ? '−' : '').$d($centavos);
    @endphp
    <style>
        ...
        .cifras .valor { text-align: right; white-space: nowrap; padding-left: 10pt; }
...
<table>
    <tr>
        <td class="mitad">
            <table class="cifras">
                <tr><td>Efectivo al comienzo</td><td class="valor">{{ $d($hoja->efectivo_inicial_centavos) }}</td></tr>
                <tr><td>Ventas en efectivo (según lectura)</td><td class="valor">{{ $d($hoja->ventas_lectura_centavos) }}</td></tr>
                <tr class="calculado"><td>Total venta y cambio</td><td class="valor">{{ $d($t['venta_y_cambio']) }}</td></tr>
        ...
        <td class="mitad">
            <table class="cifras">
                <tr><td>ATH / Visa / Mastercard</td><td class="valor">{{ $d($hoja->tarjeta_centavos) }}</td></tr>
                <tr><td>ATH Móvil</td><td class="valor">{{ $d($hoja->ath_movil_centavos) }}</td></tr>
                <tr class="calculado"><td>Total efectivo</td><td class="valor">{{ $d($t['total_efectivo']) }}</td></tr>
            </table>
        </td>
    </tr>
</table>
...
<table class="gastos">
    ...
    <tbody>
        @forelse ($hoja->gastos as $gasto)
            <tr><td>{{ $gasto->descripcion }}</td><td class="valor">{{ $d($gasto->monto_centavos) }}</td></tr>
        @empty
...
<table class="cifras">
    ...
    <tr class="total">
        ...
        <td class="valor {{ $t['a_depositar'] < 0 ? 'descuadre' : '' }}">{{ $d($t['a_depositar']) }}</td>
    </tr>
    <tr><td>Total ventas</td><td class="valor">{{ $d($t['total_ventas']) }}</td></tr>
</table>
...
<table class="cifras">
    <tr>
        <td>Cobrado en efectivo ({{ $facturado['facturas'] }} factura(s))</td>
        <td class="valor">{{ $d($facturado['efectivo']) }}</td>
        <td class="valor {{ $comparacion['efectivo']['cuadra'] ? '' : 'descuadre' }}" style="width: 22%;">
            {{ $comparacion['efectivo']['cuadra'] ? 'cuadra' : 'diferencia '.$d($comparacion['efectivo']['diferencia']) }}
        </td>
    </tr>
    <tr>
        <td>Cobrado con tarjeta</td>
        <td class="valor">{{ $d($facturado['tarjeta']) }}</td>
        <td class="valor {{ $comparacion['tarjeta']['cuadra'] ? '' : 'descuadre' }}">
            {{ $comparacion['tarjeta']['cuadra'] ? 'cuadra' : 'diferencia '.$d($comparacion['tarjeta']['diferencia']) }}
        </td>
    </tr>
    <tr>
        <td>Cobrado con ATH Móvil</td>
        <td class="valor">{{ $d($facturado['ath_movil']) }}</td>
        <td class="valor {{ $comparacion['ath_movil']['cuadra'] ? '' : 'descuadre' }}">
            {{ $comparacion['ath_movil']['cuadra'] ? 'cuadra' : 'diferencia '.$d($comparacion['ath_movil']['diferencia']) }}
        </td>
    </tr>
</table>
